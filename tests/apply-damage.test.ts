...
describe('applyDamage', () => {
  const createGameState = (activePokemon: any = null): GameState => ({
    activePokemon,
    bench: [],
    prizeCards: 6,
    discardPile: [],
  ...
  describe('Error Cases', () => {
    it('should throw error when there is no active pokemon', () => {
      const gameState = createGameState(null);
      
      expect(() => applyDamage(gameState, 10)).toThrow(
        ERROR_MESSAGES.NO_ACTIVE_POKEMON
      );
    });
    ...
    it('should throw error when damage is zero', () => {
      ...
      expect(() => applyDamage(gameState, 0)).toThrow(
  ...
  describe('Damage Application without Knockout', () => {
    it('should apply damage to pokemon without existing damage counters', () => {
      const gameState = createGameState(mockPokemon.pikachu);
      const damage = 10;

      const result = applyDamage(gameState, damage);
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle consecutive damage applications', () => {
      ...
      gameState = applyDamage(gameState, 20);
      expect(gameState.activePokemon?.damageCounters).toBe(20);

      gameState = applyDamage(gameState, 30);
      expect(gameState.activePokemon?.damageCounters).toBe(50);

      gameState = applyDamage(gameState, 30); // 80 - 50 - 30 = 0
